...
[[ -f "${ENV_FILE}" && -r "${ENV_FILE}" && ! -L "${ENV_FILE}" ]] \
  || die "${ENV_FILE} falta ou não pode ser lido. Execute 'make init'."
[[ -r "${EXAMPLE_FILE}" ]] || die "Falta o exemplo do serviço: ${EXAMPLE_FILE}"
[[ "$(stat -c '%a' "${ENV_FILE}")" == 600 ]] \
  || die "${ENV_FILE} tem de ter modo 0600."
...
for file in "${ENV_FILE}" "${EXAMPLE_FILE}"; do
  ...
  [[ -z "${duplicate}" ]] \
    || die "atribuição activa duplicada em ${file}: ${duplicate//$'\n'/, }"
done
...
if ((${#added_keys[@]} > 0)); then
  ...
  printf 'Chaves adicionadas:\n'
  printf '  %s\n' "${added_keys[@]}"
  printf 'Backup criado.\n'
else
  printf 'Não faltam chaves; o ficheiro de ambiente não foi alterado.\n'
fi
...
if ((${#obsolete_keys[@]} > 0)); then
  printf 'Chaves locais potencialmente obsoletas:\n'
  printf '  %s\n' "${obsolete_keys[@]}" | sort
fi
